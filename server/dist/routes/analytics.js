...
router.get("/dashboard", verifyToken, async (req, res) => {
    try {
        ...
        const completedSessions = await pool.query("SELECT COUNT(*) as total FROM sessions WHERE counselor_id = $1 AND status = $2", [user_id, "completed"]);
        const commonIssues = await pool.query("SELECT topic, COUNT(*) as count FROM sessions WHERE counselor_id = $1 GROUP BY topic ORDER BY count DESC LIMIT 5", [user_id]);
        res.json({
